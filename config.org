...
** DEPRECATED Make presentations
CLOSED: [2023-09-28 Thu 03:12]
:LOGBOOK:
- State "DEPRECATED" from              [2023-09-28 Thu 03:12] \\
  Use the version in package, so that each org file can be exported as a presentation.
:END:
...
** Kitty
*** Select org-babel code block if possible
#+begin_src emacs-lisp
(defadvice mark-defun (around my-mark-defun activate)
  (if (and (eq major-mode 'org-mode) (org-in-src-block-p))
      (org-babel-mark-block)
    ad-do-it))
#+end_src

*** Send text to the nix-repl tab
#+begin_src emacs-lisp
(defun yejun/send-region-to-nix-repl ()
  (interactive)
  (shell-command-on-region
   (if (use-region-p) (region-beginning) (point-min))
   (if (use-region-p) (region-end) (point-max))
  ...
  (if (and (eq major-mode 'org-mode)
           (let ((block-info (org-element-at-point)))
             (and (eq (car block-info) 'src-block)
                  (string= (org-element-property :language block-info) "nix"))))
      (yejun/send-region-to-nix-repl)
    ad-do-it))
#+end_src
